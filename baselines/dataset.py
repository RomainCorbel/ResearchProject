...
def Dataset(set, norm=False, coef_norm=None, crop=None, sample=None,
            n_boot=int(5e5), surf_ratio=.1,
            surface_only=False, pressure_only=False):
    ...
    base_dir = osp.dirname(__file__)  # ensures we use baselines/Dataset regardless of cwd

    for k, s in enumerate(tqdm(set)):
        # ---- robust paths under baselines/Dataset ----
        case_dir = osp.join(base_dir, 'Dataset', s)
        internal_path = osp.join(case_dir, f'{s}_internal.vtu')
        aerofoil_path = osp.join(case_dir, f'{s}_aerofoil.vtp')
        if not (osp.exists(internal_path) and osp.exists(aerofoil_path)):
            raise FileNotFoundError(
                f"Missing files for case '{s}'.\n"
                f"Expected:\n  {internal_path}\n  {aerofoil_path}"
            )
        ...
        if sample is not None:
            ...
            surf_tgt  = surf_sampled_points[:, 7:]

            # tensors + surf flags
            surf = torch.cat([
                torch.zeros(len(pos), dtype=torch.bool),
                torch.ones(len(surf_pos), dtype=torch.bool)
            ], dim=0)
            pos = torch.cat([torch.tensor(pos, dtype=torch.float),
                             torch.tensor(surf_pos, dtype=torch.float)], dim=0)
            x   = torch.cat([torch.tensor(init, dtype=torch.float),
                             torch.tensor(surf_init, dtype=torch.float)], dim=0)
            y   = torch.cat([torch.tensor(target, dtype=torch.float),
                             torch.tensor(surf_tgt, dtype=torch.float)], dim=0)
            ...
            # tensors + surf flags
            surf = torch.tensor(surf_bool, dtype=torch.bool)
            pos  = torch.tensor(pos,    dtype=torch.float)
            x    = torch.tensor(init,   dtype=torch.float)
            y    = torch.tensor(target, dtype=torch.float)

        # ---- surface-only / pressure-only filters ----
        if surface_only:
            pos  = pos[surf]
            x    = x[surf]
            y    = y[surf]
            surf = torch.ones(pos.size(0), dtype=torch.bool)

        if pressure_only:
            y = y[:, 2:3]  # keep only pressure

        # ---- online means on what we'll actually use ----
        if norm and coef_norm is None:
            if k == 0:
                old_length = x.shape[0]
                mean_in  = x.double().mean(dim=0).numpy()
                mean_out = y.double().mean(dim=0).numpy()
            else:
                new_length = old_length + x.shape[0]
                mean_in  += (x.double().sum(dim=0).numpy() - x.shape[0]*mean_in)  / new_length
                mean_out += (y.double().sum(dim=0).numpy() - x.shape[0]*mean_out) / new_length
                old_length = new_length

        # ---- graph object ----
        dataset.append(Data(pos=pos, x=x, y=y, surf=surf))

    # ---- finalize normalization ----
    if norm and coef_norm is None:
        # compute std across the dataset
        ...
        for i, d in enumerate(dataset):
            ...
            if i == 0:
                ...
                new_len = old_length + xi.shape[0]
                std_in  += (((xi - mean_in )**2).sum(axis=0, dtype=np.double) - xi.shape[0]*std_in )/new_len
                std_out += (((yi - mean_out)**2).sum(axis=0, dtype=np.double) - xi.shape[0]*std_out)/new_len
                old_length = new_len
        ...
        for d in dataset:
            d.x = (d.x - mean_in )/(std_in  + 1e-8)
            d.y = (d.y - mean_out)/(std_out + 1e-8)
